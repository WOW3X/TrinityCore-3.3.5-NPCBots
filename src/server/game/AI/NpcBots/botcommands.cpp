...
public:
    ...
    static bool HandleNpcBotSpawnCommand(ChatHandler* handler, const char* args)
    {
        ...
        if (!creature->Create(sObjectMgr->GenerateCreatureSpawnId()/* GenerateLowGuid(HIGHGUID_UNIT)*/, map, chr->GetPhaseMaskForSpawn(), id, x, y, z, o))
        {
            delete creature;
            return false;
        }
 ...
 //       Creature* bot = ObjectAccessor::GetCreatureOrPetOrVehicle(*player, sel);//GetObjectInWorld(sel, (Creature*)NULL);
 ...
 //   }
    ...
    //        return false;
    //    }
    //    if (guid.IsPlayer()) //IS_PLAYER_GUID(guid))
    //    {
    //        master = owner;
    //        all = true;
    //    }
    //    else if (guid.IsCreature()) //IS_CREATURE_GUID(guid))
    //    {
