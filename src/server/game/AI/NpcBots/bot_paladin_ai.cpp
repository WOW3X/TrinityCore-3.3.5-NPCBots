...
public:
    ...
    struct paladin_botAI : public bot_minion_ai
    {
       ...
       /*else */if (spellInfo->GetAllEffectsMechanicMask() & (1<<MECHANIC_SNARE) ||
        ...
        void EnterEvadeMode(EvadeReason why = EVADE_REASON_OTHER) override { bot_minion_ai::EnterEvadeMode(why); }
        ...
        bool BuffTarget(Unit* target, uint32 diff)
        {
            ...
            bool wisdom = (mask & SPECIFIC_BLESSING_WISDOM) != 0;
            bool kings = (mask & SPECIFIC_BLESSING_KINGS) != 0;
            bool sanctuary = (mask & SPECIFIC_BLESSING_SANCTUARY) != 0;
            bool might = (mask & SPECIFIC_BLESSING_MIGHT) != 0;
 ...
 /*SPECIAL*/InitSpellMap(ARDENT_DEFENDER_HEAL, true);
        ...
        void ApplyClassPassives()
        {
            ...
            RefreshAura(PP_PURE, level >= 55 ? 1 : 0);
            RefreshAura(PP_WISE, level >= 35 ? 1 : 0);
    ...
    private:
        ...
        //Talents
            ...
            PP_PURE/*Judgements of the Pure*/   = 54155,
            PP_WISE/*Judgements of the Wise*/   = 31878,
