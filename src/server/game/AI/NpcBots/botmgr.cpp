...
uint8 BotMgr::GetNpcBotsCount(bool inWorldOnly) const
{
    ...
    for (BotMap::const_iterator itr = _bots.begin(); itr != _bots.end(); ++itr)
        if (ObjectAccessor::GetCreature(*_owner, itr->first))
            ++count;
    return count;
}
...
Creature* BotMgr::GetBot(ObjectGuid const& guid) const
{
...
void BotMgr::CleanupsBeforeBotDelete(ObjectGuid const& guid)
{
    BotMap::const_iterator itr = _bots.find(guid);
...
void BotMgr::_addBotToRemoveList(ObjectGuid const& guid)
{
...
void BotMgr::RemoveBot(ObjectGuid const& guid, uint8 removetype)
{
    BotMap::const_iterator itr = _bots.find(guid);
