...
void bot_ai::ResetBotAI(uint8 resetType)
{
    ...
    if (!me->IsInWorld())
    {
        ...
        teleHomeEvent->ScheduleAbort();// to_Abort = true; //make sure event will not be executed twice
...
void bot_minion_ai::BuffAndHealGroup(Player* gPlayer, uint32 diff)
{
    ...
    if (IAmFree())
    {
        ...
        {
            if (Unit* u = ObjectAccessor::GetUnit(*me, ObjectGuid(*itr)))
            {
...
void bot_minion_ai::RezGroup(uint32 REZZ, Player* gPlayer)
{
    ...
    if (IAmFree())
    {
        ...
        {
            if (Player* player = playerOrCorpse->GetTypeId() == TYPEID_PLAYER ? playerOrCorpse->ToPlayer() : ObjectAccessor::FindConnectedPlayer(playerOrCorpse->ToCorpse()->GetOwnerGUID()))
...
bool bot_ai::IsInBotParty(Unit const* unit) const
{
    ...
    if (Creature const* cre = unit->ToCreature())
    {
        ...
        else
        {
            ...
            if (Group* gr = master->GetGroup())
                if (gr->IsMember(unit->GetOwnerGUID()))
                    return true;
        }
    }
...
void bot_minion_ai::Evade(bool force)
{
    ...
    if (master->IsInCombat() && !IAmFree())
    {
        if (!master->getHostileRefManager().isEmpty())
        {
            ...
            for (std::set<ObjectGuid>::const_iterator i = Set.begin(); i != Set.end(); ++i)
            {
                Unit* unit = ObjectAccessor::GetUnit(*me, *i);
    ...
    if (player->GetGUID().GetCounter() != creature->GetBotAI()->GetBotOwnerGuid())
    {
        if (creature->IsFreeBot() && !player->IsGameMaster())
        {
            ...
            if (!reason)
            {
                ...
                AddGossipItemFor(player, GOSSIP_ICON_TAXI, "Will you follow me? (hire)", GOSSIP_SENDER_HIRE, GOSSIP_ACTION_INFO_DEF + reason);
...
bool bot_minion_ai::OnGossipSelect(Player* player, Creature* creature/* == me*/, uint32 sender, uint32 action)
{
    ...
    switch (sender)
    {
        ...
        case GOSSIP_SENDER_EARLYDISMISS: //thesawolf - early dismissal/delete
        {
...
void bot_minion_ai::_updateEquips(uint8 slot, Item* item)
{
    ...
    for (k = 0; k != BOT_INVENTORY_SIZE; ++k)
    {
        if (Item* botitem = _equips[k])
        {
            ...
            stmt->setUInt32(++index, botitem->GetGuidValue(ITEM_FIELD_GIFTCREATOR).GetCounter());//GUID_LOPART(botitem->GetUInt64Value(ITEM_FIELD_GIFTCREATOR)));
...
void bot_ai::BotSpeak(std::string const& text, uint8 msgtype, uint32 language, ObjectGuid speaker, ObjectGuid receiver)
{
    ...
    if (msgtype == CHAT_MSG_WHISPER)
    {
        ASSERT(!receiver.IsEmpty() && "BotSpeak(): no receiver for whisper!");
    ...
    {
        if (Unit* snd = ObjectAccessor::FindConnectedPlayer(speaker))//ObjectAccessor::FindUnit(speaker))
        {
...
{
    if (!damageInfo.GetVictim() || !damageInfo.GetVictim()->IsAlive() || damageInfo.GetVictim() == me)
        return;
...
void bot_minion_ai::CastBotItemCombatSpell(DamageInfo const& damageInfo, Item* item, ItemTemplate const* proto)
{
    ...
    if (canTrigger/*damageInfo.GetDamage() > 0*/) //if (procVictim & PROC_FLAG_TAKEN_DAMAGE)
    {
        for (uint8 i = 0; i != MAX_ITEM_PROTO_SPELLS; ++i)
        {
            _Spell const& spellData = proto->Spells[i];
    ...
    for (uint8 e_slot = 0; e_slot != MAX_ENCHANTMENT_SLOT; ++e_slot)
    {
        ...
        for (uint8 s = 0; s != MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
        {
            ...
            if (entry && entry->procEx)
            {
                ...
                if ((entry->procEx & damageInfo.GetHitMask()) == 0)//procEx) == 0)
                    continue;
            }
            else
            {
            ...
            if (roll_chance_f(chance))
            {
                if (spellInfo->IsPositive())
                    ...
                    me->CastSpell(damageInfo.GetVictim(), spellInfo, true, item);
            }
        }
    }
}
...
void bot_minion_ai::AbortTeleport()
{
    if (teleHomeEvent)
    {
        teleHomeEvent->ScheduleAbort();//to_Abort = true;
        teleHomeEvent = NULL;
    }

    if (teleFinishEvent)
    {
        teleFinishEvent->ScheduleAbort();//to_Abort = true;
        teleFinishEvent = NULL;
    }
...
void bot_ai::BotJumpInPlaceInFrontOf(Position* pos, float speedXY, float maxHeight)
{
    ...
    //me->AttackStop();
    me->BotStopMovement();
    me->GetMotionMaster()->MoveJump(x, y, z, 0.0f, speedXY, maxHeight);
}
