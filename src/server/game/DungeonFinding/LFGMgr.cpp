            ...
            for (GroupReference* itr = grp->GetFirstMember(); itr != NULL && joinData.result == LFG_JOIN_OK; itr = itr->next())
            {
                if (Player* plrg = itr->GetSource())
                {
                    ...
                    for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
                    {
                        ...
                        if (Creature* bot = itr->second)//ObjectAccessor::GetCreature(itr->second, itr->first))
                        {
                            if (!bot->IsTempBot())
                            {
        ...
        for (GroupReference* itr = grp->GetFirstMember(); itr != NULL; itr = itr->next())
        {
            if (Player* plrg = itr->GetSource())
            {
                ...
                for (BotMap::const_iterator itr = map->begin(); itr != map->end(); ++itr)
                {
                    ...
                    if (Creature* bot = itr->second)//ObjectAccessor::GetObjectInWorld(itr->first, (Creature*)NULL))
                    {
                        if (!bot->IsTempBot())
                        {
